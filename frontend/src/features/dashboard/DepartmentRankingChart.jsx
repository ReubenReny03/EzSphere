...
import { useScoringDepartments } from '@/api/hooks/useScores';

export const DepartmentRankingChart = () => {
  const { data, isLoading } = useScoringDepartments();
  const rows = (data?.data || []).map((d) => ({
    name: d.department.code,
    totalScore: Math.round(d.totalScore),
  }));
  ...
  return (
    <Card title="Department ESG Ranking">
      {isLoading ? (
        ...
        <EmptyState message="No department scores yet" />
      ) : (
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={rows}>
            ...
            />
            <Bar dataKey="totalScore" fill="#3b82f6" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </Card>
